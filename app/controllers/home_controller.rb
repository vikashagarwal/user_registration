class HomeController < ApplicationController
  before_action :set_user, only: %i[edit update]
    ...
    redirect_to edit_home_path(current_user.id) unless current_user.is_admin
  ...
  def update
    if @user.update(user_params)
      ...
      redirect_to(current_user.is_admin ? home_index_path : edit_home_path(current_user.id), notice: 'User was successfully updated.')
    else
      flash[:error] = @user.errors.full_messages.join(', ')
